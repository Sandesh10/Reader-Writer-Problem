...
//checks whether the process needs to wait
int wait(struct sema *s){
	struct sema *P=head,*pp=front,*b=NULL;
	if(s->name=='w'){
		if(!head)return 0;
		else{
            if(!front){front=rear=s;s->next=NULL;}
            else{
                while(pp&&pp->name=='w'){
                    b=pp;pp=pp->next;
                }
		...
		if((head&&head->name=='w')||(front&&front->name=='w'))
		{
			if(!front){
...
void signal(struct sema *PP){
	...
	if(!head){head=PP,PP->next=NULL;}
	else{
		while(p){
			b=p;
			p=p->next;
		}
		b->next=PP; PP->next=NULL;
	}
}
...
void finish(){
    ...
    if(!p){
		...
		if(!pre){head=head->next;p->next=NULL;p=NULL;}
